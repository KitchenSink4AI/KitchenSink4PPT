...
2. OPT-OUT IS TOTAL. KS4P_NO_UPDATE_CHECK does not merely hide the line;
...
4. ONE SURFACE, ONLY WHEN BEHIND. diagnose carries the line when a
...
def test_opt_out_hides_a_real_pending_update(monkeypatch):
    ...
    monkeypatch.setenv(uc.OPT_OUT_ENV, "1")
    assert uc.update_notice() is None
    assert "update" not in _call("diagnose")
...
def test_diagnose_carries_the_line_only_when_behind():
    _write_cache(__version__)
    assert "update" not in _call("diagnose")
    _write_cache("999.0.0")
    out = _call("diagnose")
    assert out["update"] == uc.update_notice()
    # the environment report itself is untouched
    assert out["server"] == "kitchensink4ppt"
    assert "surface" in out and "engines" in out


def test_no_other_tool_mentions_the_update():
    """The line lives in diagnose and nowhere else."""
    _write_cache("999.0.0")
    assert uc.update_notice() is not None
    for result in (_call("get_workflows"),
                   _call("get_workflows", "build-a-diagram"),
                   _call("enable_tools", []),
                   _call("disable_tools", [])):
...
def test_cache_lives_in_the_state_dir_not_beside_documents(monkeypatch):
    ...
    path = uc.cache_path()
    assert path.parent.name == "pptx-mcp"

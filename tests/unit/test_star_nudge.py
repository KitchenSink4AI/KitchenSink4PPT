...
from kitchensink4ppt.core.star_nudge import (
    MARKER_NAME,
    MESSAGE,
    announce_once,
    marker_path,
...
def state(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KS4P_UPDATE_CACHE_DIR", str(tmp_path))
    monkeypatch.delenv("KS4P_STAR_NUDGE", raising=False)
    return tmp_path


def test_the_wording_is_exactly_what_was_approved():
    assert MESSAGE == (
        "KitchenSink4PPT is community-supported. If it helps you, star "
        "github.com/KitchenSink4AI/KitchenSink4PPT. Shown once."
    )
...
def test_the_toggle_silences_it(state, monkeypatch):
    monkeypatch.setenv("KS4P_STAR_NUDGE", "off")
    out = io.StringIO()
...
def test_an_unwritable_state_directory_is_silence_not_a_crash(state, monkeypatch):
    monkeypatch.setattr(
        "kitchensink4ppt.core.star_nudge.marker_path",

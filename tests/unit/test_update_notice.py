...
4. THE OFF SWITCH. KS4P_UPDATE_CHECK=off produces a report saying the
...
6. ONE SURFACE. diagnose is the only tool that can reach the
...
def test_diagnose_carries_the_report(monkeypatch):
    monkeypatch.setattr(uc, "_fetch", _Counter(_payload("999.0.0")))
    info = _call("diagnose")
...
def test_off_is_visible_through_the_server_surface(monkeypatch):
    monkeypatch.setenv(uc.OFF_ENV, "off")
    monkeypatch.setattr(uc, "_fetch", _landmine)
    assert _call("diagnose")["update_check"]["state"] == "disabled"
...
def test_no_other_tool_path_fires_the_check(tmp_path, monkeypatch):
    ...
    _call("get_presentation_info", str(deck))
    _call("list_elements", str(deck), "slides")
    _call("get_workflows")
...
def test_the_request_carries_no_body_and_no_identifier(monkeypatch):
    ...
    assert "kitchensink4ppt" in values          # package and version only

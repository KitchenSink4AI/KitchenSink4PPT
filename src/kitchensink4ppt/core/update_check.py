"""Is a newer KitchenSink4PPT published? A check-on-demand version report.
...
- OFF SWITCH: KS4P_UPDATE_CHECK=off disables the whole thing (no network
  call, no cache read, no cache write); the surface then says so. The
  older KS4P_NO_UPDATE_CHECK=1 still disables it too.
...
CACHE_DIR_ENV = "KS4P_UPDATE_CACHE_DIR"
...
INSTALL_NOTE = (
    ...
    "with pip install -U kitchensink4ppt. The server never downloads, "
...
PRIVACY_NOTE = (
    ...
    "nothing else off the machine, ever. Set KS4P_UPDATE_CHECK=off to turn the check "
    "off.")
